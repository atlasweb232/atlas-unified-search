export class AssistantActionService {
  ...
  async run({ tenantId, userId, searchRunId, actionType, selectedResultIds = [], prompt = '', provider = '' }) {
    ...
    try {
      this.store.updateAssistantAction(action.id, { status: 'running' });
      ...
      const updated = this.store.updateAssistantAction(action.id, { status: 'completed', responseText: response.text, artifactIds, completedAt: new Date().toISOString(), citations: response.citations || [] });
      this.store.audit({ eventType: 'assistant_action_complete', tenantId, userId, metadata: { actionId: action.id, actionType, artifactIds } });
      await this.store.save();
      return updated;
    } catch (error) {
      const updated = this.store.updateAssistantAction(action.id, { status: 'failed', error: error.message, completedAt: new Date().toISOString() });
